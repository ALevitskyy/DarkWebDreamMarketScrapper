...
def get_text():
        ...
        final_im = change_contrast(
            Image.fromarray(
                ndimage.filters.gaussian_filter(
                    Image.open("cleaned.png"), i, mode="nearest"
                )
            ),
            z,
        )
    ...
    try:
        ...
        return (
            Counter(first).most_common(1)[0][0]
            + Counter(second).most_common(1)[0][0]
            + Counter(third).most_common(1)[0][0]
            + Counter(fourth).most_common(1)[0][0]
        )
    except:
        return "fail"
